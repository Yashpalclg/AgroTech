...
# Get the color and size you want
t_color = 'white'  # color for all text
title_size = '40px'

# Apply styling and size to the title
styled_title = f'<span style="color: {t_color}; font-size: {title_size};">**AgroTech**</span>'
...
def main():
    ...
    # Apply styling and size to the title
    styled_subheader = f'<span style="color: {t_color}; font-size: {subheader_size};">**Crop Recommendation App**</span>'
...
# Get the color and size you want
subheader2_size = '30px'
# Apply styling and size to the title
styled_subheader2 = f'<span style="color: {t_color}; font-size: {subheader2_size};">**Pest Image Classification**</span>'
...
if upload is not None:
    ...
    if st.button('Predict Pest', key='Pest'):
        ...
        # Get the color you want (e.g., 'red')
        text_size = '40px'
        # Apply styling to the last Markdown text
        last_result = map_result[np.argmax(result)]
        styled_result = f'<span style="color: {t_color};font-size: {text_size};">**{last_result}**</span>'
        st.markdown(styled_result, unsafe_allow_html=True)

        detail_result = {
            0: '**Pesticides Recommended**  = *Carbaryl, Malathion ,Permethrin ,Cyfluthrin*',
        ...
        # Apply styling to the last Markdown text
        last_result2 = detail_result[np.argmax(result)]
        styled_result2 = f'<span style="color: {t_color};font-size: {text2_size};">{last_result2}</span>'
        st.markdown(styled_result2, unsafe_allow_html=True)
